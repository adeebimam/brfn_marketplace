...
class Product(models.Model):
    ...
    estimated_unit_weight_kg = models.DecimalField(
        max_digits=6,
        decimal_places=2,
        default=Decimal("0.00"),
        help_text="Estimated weight per unit in kg for surplus analytics.",
    )
...
class FavouriteProducer(models.Model):
    ...
    producer = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="favourited_by_customers",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.customer} -> {self.producer}"


class SurplusDealNotification(models.Model):
    customer = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="surplus_deal_notifications",
    )
    producer = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="sent_surplus_deal_notifications",
    )
    product = models.ForeignKey(
        Product,
        on_delete=models.CASCADE,
        related_name="surplus_notifications",
    )
    message = models.CharField(max_length=255)
...
class SurplusAnalyticsRecord(models.Model):
    class RecordType(models.TextChoices):
        SAVED = "SAVED", "Saved from waste"
        UNSOLD = "UNSOLD", "Unsold expired surplus"

    producer = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="surplus_analytics_records",
    )
    product = models.ForeignKey(
        Product,
        on_delete=models.CASCADE,
    ...
    customer_saving = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=Decimal("0.00"),
    )
    ...
    def __str__(self):
        return (
            f"{self.get_record_type_display()} - {self.product.name} "
            f"({self.quantity} units, {self.estimated_weight_kg} kg)"
        )
